...
while [ $# -gt 0 ] ; do
  case "$1" in
    ...
    -vcd)
      ...
      VCD="<app>-sim-chip-route-${OPCOND}.vcd"
      ;;
    -saif)
      [ -z "$OPCOND" ] && Usage    # -saif requies -min|-max before it
      SAIF="<app>-sim-chip-route-${OPCOND}-" # will be appended with "${Mode}.saif"
      ;;
    *)
...
if [ -n "$VCD" ] ; then
  ...
  # Value Change Dump
  vcd file $VCD
  vcd comment "<App> Reconf. Ex.App sim-chip-route"
