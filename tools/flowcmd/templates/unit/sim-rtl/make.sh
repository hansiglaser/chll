...
case "$SIM_MODE" in
  sim-rtl)
    #$# CompileXyz
    #$# vcom ${VCOM_OPTS} -work work -87    $APPVHDL_PATH/unit.vhd
    #$# vcom ${VCOM_OPTS} -work work -93    $APPVHDL_PATH/unit.vhd
    #$# vlog ${VLOG_OPTS} -work work        $APPVLOG_PATH/spifsm.v
    #$# vcom ${VCOM_OPTS} -work work -2002  $APPTEST_PATH/unit_tb.vhd
    ;;
  sim-abc)
    #$# CompileXyz
    ;;
