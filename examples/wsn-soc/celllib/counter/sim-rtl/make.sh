...
# compile all files
vlog ${VLOG_OPTS} -work work        $CELLVLOG_PATH/counter.v
vlog ${VLOG_OPTS} -work work        $CELLVLOG_PATH/counter_tv1.v
vlog ${VLOG_OPTS} -work work        $CELLVLOG_PATH/counter_rv1.v
vcom ${VCOM_OPTS} -work work -2002  $TRFSMTEST_PATH/tbfuncs-p.vhd
vlog ${VLOG_OPTS} -work work        $CELLTEST_PATH/counter-verilog-c.v
vcom ${VCOM_OPTS} -work work -2002  $CELLTEST_PATH/counter_tb.vhd
