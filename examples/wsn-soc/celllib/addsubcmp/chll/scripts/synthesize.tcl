...
set RedVars [list "Greater" "Less" "Add"]
...
set RedVarsImpl(Greater) [list "Direct"]
set RedVarsImpl(Less)    [list "Direct"]
set RedVarsImpl(Add)     [list "Direct"]
...
read_verilog $VLOG_DIR/${CELL_NAME_LC}_greater.v
read_verilog $VLOG_DIR/${CELL_NAME_LC}_less.v
read_verilog $VLOG_DIR/${CELL_NAME_LC}_add.v
...
foreach TopVar $TopVars {
  ...
  flatten "miter"
  sat -prove-asserts -verify -enable_undef -show-inputs -show-outputs "miter"
  delete "miter"
}
...
foreach RedVar $RedVars {
  foreach Impl $RedVarsImpl($RedVar) {
    ...
    flatten "miter"
    sat -prove-asserts -verify -enable_undef -show-inputs -show-outputs -max 10 "miter"
    delete "miter"
  }
}
