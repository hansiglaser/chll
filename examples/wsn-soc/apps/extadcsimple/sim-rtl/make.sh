...
function CompileCore() {
  ...
  vlog ${VLOG_OPTS} -work work        $CORE_PATH/verilog/master.v
...
case "$SIM_MODE" in
  ...
  sim-yosys-trfsm)
    ...
    CompileTRFSM
    # wrapper and new module
    vcom ${VCOM_OPTS} -work work -93   $APPCHLL_PATH/${APP_NAME_LC}-fsm-fsm-wrapper.vhd
    # top module
  ...
  sim-yosys-extract-trfsm)
    ...
    CompileCellLib
    # wrapper and new module
    vcom ${VCOM_OPTS} -work work -93    $APPCHLL_PATH/${APP_NAME_LC}-extract-fsm-wrapper.vhd
    # top module
