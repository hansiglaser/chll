...
if [ "$SIM_MODE" != "sim-chip-fpga" ] ; then
  CompileBusMaster
fi
...
case "$SIM_MODE" in
  sim-rtl)
    ...
    vlog ${VLOG_OPTS} -work work        $APPTEST_PATH/${APP_NAME_LC}-verilog-c.v
    ;;
  sim-yosys)
...
# MAX6682 sensor model
vcom ${VCOM_OPTS} -work work -2002  $APPTEST_PATH/${APP_NAME_LC}-model.vhd
vcom ${VCOM_OPTS} -work work -2002  $APPTEST_PATH/${APP_NAME_LC}-model_tb.vhd
# testbench
