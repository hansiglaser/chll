...
while [ $# -gt 0 ] ; do
  case "$1" in
    ...
    -vcd)
      ...
      VCD="adt7310p32ls16-sim-chip-route-${OPCOND}-${FREQ}.vcd"
      ;;
    -saif)
      [ -z "$OPCOND" ] && Usage    # -saif requies -min|-max before it
      [ -z "$FREQ"   ] && Usage    # -saif requies -100kHz|10MHz before it
      SAIF="adt7310p32ls16-sim-chip-route-${OPCOND}-${FREQ}-" # will be appended with "${Mode}.saif"
      ;;
    *)
...
if [ -n "$VCD" ] ; then
  ...
  # Value Change Dump
  vcd file $VCD
  vcd comment "ADT7310P32LS16 Reconf. Ex.App sim-chip-route"
